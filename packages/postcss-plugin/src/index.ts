              ...
              if (!isNaN(baseValue)) {
                ...
                if (generateCustomProperties) {
                  ...
                  if (root) {
                    ...
                    if (!rootRule || rootRule.type !== 'rule' || rootRule.selector !== ':root') {
                      rootRule = root.rule({ selector: ':root' });
                    ...
                    mockConfig.breakpoints.forEach(breakpoint => {
                      if (breakpoint.name !== mockConfig.base.name) {
                        ...
                        const mediaRule = root.rule({ selector: `@media ${mediaQuery}` });
                        const innerRootRule = mediaRule.rule({ selector: ':root' });
                        
                        innerRootRule.append({
                          prop: propName,
