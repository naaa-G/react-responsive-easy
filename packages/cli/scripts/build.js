...
/**
 * Enterprise-grade build script for CLI Package
...
function main() {
  ...
  // Step 2: Pre-build steps
  log(`\n${colors.yellow}🔧 Running pre-build steps...${colors.reset}`);
  const prebuildStart = Date.now();
  execCommand('pnpm clean && pnpm type-check', { stdio: 'inherit' });
  const prebuildTime = Date.now() - prebuildStart;
  log(`${colors.green}✅ Pre-build steps completed in ${prebuildTime}ms${colors.reset}`);
  ...
  const cliExists = fs.existsSync('dist/cli.js');
  const mainExists = fs.existsSync('dist/index.js');
  const esmExists = fs.existsSync('dist/index.esm.js');
  
  if (distExists && typesExist && cliExists && mainExists && esmExists) {
    ...
    log(`${colors.green}   • Pre-build steps: ${prebuildTime}ms${colors.reset}`);
    ...
    log(`${colors.green}✅ CLI executable built${colors.reset}`);
    log(`${colors.green}✅ CommonJS and ESM formats built${colors.reset}`);
    log(`${colors.green}✅ Type checking passed${colors.reset}`);
    ...
    if (!cliExists) log(`${colors.red}  - dist/cli.js${colors.reset}`);
