...
/**
 * Enterprise-grade build script for Next Plugin Package
...
function main() {
  ...
  execCommand('tsc', { stdio: 'inherit' });
  const tscTime = Date.now() - tscStart;
  log(`${colors.green}✅ Main package built in ${tscTime}ms${colors.reset}`);
  ...
  if (distExists && mainExists && typesExist) {
    ...
    log(`${colors.green}   • Main package: ${tscTime}ms${colors.reset}`);
    ...
    log(`${colors.green}✅ Next.js plugin compiled${colors.reset}`);
